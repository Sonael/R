...
Em *Details* do app criado e ir na aba *Keys and Tokens* para conseguir suas chaves de acesso.
...
*Com suas chaves em mão, crie as variaveis referente a cada uma das chaves e atribua a elas o valores correspondentes: *
...
*Pontuando os tweets usando a função `get_nrc_sentiment` do pacote "syuzhet": * `s <- get_nrc_sentiment(tweets)`
    ...
    Esse pacote quebra as emoções em 8 tipos diferentes(raiva, antecipação, nojo, medo, alegria, tristesa, surpresa,  confiança) e os sentimentos em dois tipos ( negativo, positivo).
